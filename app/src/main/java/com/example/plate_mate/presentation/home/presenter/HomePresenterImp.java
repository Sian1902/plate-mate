...
public class HomePresenterImp implements HomePresenter {
    ...
    private void setupSearchDebounce() {
        disposables.add(searchSubject.debounce(400, TimeUnit.MILLISECONDS).distinctUntilChanged().observeOn(AndroidSchedulers.mainThread()).subscribe(this::fetchSearchMeals, throwable -> {
    ...
    public void loadHomeData() {
        ...
        disposables.add(mealRepo.getCachedSplashData().subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(data -> {
            ...
            if (data != null) {
                List categories = (data.getCategories() != null && data.getCategories().getMeal() != null) ? data.getCategories().getMeal() : new ArrayList<>();
                List areas = (data.getAreas() != null && data.getAreas().getMeals() != null) ? data.getAreas().getMeals() : new ArrayList<>();
                List ingredients = (data.getIngredients() != null && data.getIngredients().getMeals() != null) ? data.getIngredients().getMeals() : new ArrayList<>();

                Log.d(TAG, "Filter options - Categories: " + categories.size() + ", Areas: " + areas.size() + ", Ingredients: " + ingredients.size());
                ...
                Meal heroMeal = null;
                if (data.getRandomMeal() != null && data.getRandomMeal().getMeals() != null && !data.getRandomMeal().getMeals().isEmpty()) {
    ...
    public void toggleFavorite(Meal meal) {
        ...
        if (favoriteMealIds.contains(meal.getIdMeal())) {
            disposables.add(mealRepo.deleteFavorite(meal.getIdMeal()).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(() -> {
            ...
            disposables.add(mealRepo.insertFavorite(meal).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(() -> {
    ...
    private void fetchCategoryMeals(String category) {
        filterDisposables.add(mealRepo.searchMealsByCategory(category).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(res -> {
    ...
    private void fetchAreaMeals(String area) {
        filterDisposables.add(mealRepo.searchMealsByArea(area).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(res -> {
    ...
    private void fetchIngredientMeals(String ingredient) {
        filterDisposables.add(mealRepo.searchMealsByIngredient(ingredient).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(res -> {
    ...
    private void fetchSearchMeals(String query) {
        filterDisposables.add(mealRepo.SearchMealsByName(query).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(res -> {
    ...
    private void updateResults() {
        ...
        updateDisposable = Observable.fromCallable(() -> {
            ...
            return result;
        }).subscribeOn(Schedulers.computation()).observeOn(AndroidSchedulers.mainThread()).subscribe(meals -> {
